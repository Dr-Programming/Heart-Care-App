...
| ⚠️ FR-OFF-007 — **implemented as a deviation, pending owner sign-off.** The Slice 7 sync engine detects and *reports* conflicts (per-record `CONFLICT` status) but does **not** overwrite: the first-stored record always wins, because all log tables are append-only/immutable and the phone is the sole writer, so a divergent payload under an existing `client_record_id` signals a client bug rather than a legitimate edit. Overwriting would mutate clinical history with no audit trail. See `docs/design/2026-07-17-sync-design.md`, Decision 3. | | |
