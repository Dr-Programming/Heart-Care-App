...
void main() {
  test('diet topic parses from the real content file (en and am)', () {
    for (final String lang in <String>['en', 'am']) {
      ...
      final Topic topic = Topic.fromJson(
        (json[3] as Map<Object?, Object?>).cast(),
      );
      expect(topic.id, 'diet');
      expect(topic.sections, isNotEmpty);
    }
  });

  test('diet quiz has exactly one correct option per question, en and am', () {
    for (final String lang in <String>['en', 'am']) {
      final List<dynamic> json = jsonDecode(
