...
void setUpWidgetTests() {
  ...
  // has no plugin implementation under `flutter test`. Answering `getAll`
  // with an empty store is enough for the initial load, and is preferable to
  // taking a direct dependency on shared_preferences just for its test
  // helper. A runtime `context.setLocale(...)` call (e.g. a language toggle
  // exercised mid-test, not just the locale `pumpApp` starts with) also
  // *writes* through this channel to persist the new choice
  // (`EasyLocalizationController._saveLocale` → `setValue`/similar) — that
  // call expects a non-null result and null-checks it, so every write-shaped
  // method answers `true` here rather than falling through to `null`.
