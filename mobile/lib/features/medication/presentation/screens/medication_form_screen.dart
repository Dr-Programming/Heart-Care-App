...
final _medicationByIdProvider = FutureProvider.autoDispose
    .family<Medication?, String>((Ref ref, String id) async {
      final List<Medication> medications = await ref
...
class _MedicationFormScreenState extends ConsumerState<MedicationFormScreen> {
  ...
  void initState() {
    ...
    if (widget.editingId == null && entry != null) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (!mounted) return;
        final MedicationFormController controller = ref.read(
  ...
  void _setInstructions(MedicationInstructions value) {
    setState(
      () => _instructions = _instructions == value
  ...
  Widget build(BuildContext context) {
    if (!_loaded) {
      final AsyncValue<Medication?> medication = ref.watch(
        _medicationByIdProvider(widget.editingId!),
      );
      return medication.when(
        ...
        data: (Medication? found) {
          WidgetsBinding.instance.addPostFrameCallback((_) async {
            if (found != null) {
              ref
                  .read(medicationFormControllerProvider.notifier)
                  .loadForEdit(found);
              _nameController.text = found.name;
              _doseController.text = _formatDose(found.doseMg);
            }

            final CaregiverNotifySettings settings = await ref
                .read(caregiverNotifyStoreProvider)
                .get(widget.editingId!);

            final MedicationInstructions instructions = await ref
                .read(medicationInstructionsStoreProvider)
                .get(widget.editingId!);
            if (mounted) {
              setState(() {
...
String _formatDose(double doseMg) => doseMg == doseMg.roundToDouble()
    ? doseMg.toStringAsFixed(0)
    : doseMg.toString();
...
class _FormBody extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final MedicationFormState state = ref.watch(
      medicationFormControllerProvider,
    );
    final MedicationFormController controller = ref.read(
      medicationFormControllerProvider.notifier,
...
class _DoseQuickPicks extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final List<MedicationLibraryEntry> matches = searchMedicationLibrary(
      medicationName,
    );
